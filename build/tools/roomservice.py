...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the matrixx snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/matrixx.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        repo_remote = repository.get("remote", "matrixx-devices")
        repo_revision = repository.get('revision')

        if repo_remote != "matrixx-devices" and not repo_remote.startswith("aosp-"):
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/matrixx.dependencies'
    syncable_repos = []
    verify_repos = []
...
def get_default_or_fallback_revision(repo_name):
    default_revision = get_remote_revision_from_snippet(".repo/manifests/snippets/matrixx.xml", "matrixx-devices")
